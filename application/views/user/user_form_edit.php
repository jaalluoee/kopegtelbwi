			...
			<div class="col-md-4 col-md-offset-4">
				<form action="" method="post">
					...
					<div class="form-group <?=form_error('username')? 'has-error' : null?>">
						...
						<input type="text" name="username" value="<?=$this->input->post('username') ?? $row->username ?>" class="form-control" placeholder="Enter Username">
						<?=form_error('username')?>
					</div>
					...
					<div class="form-group <?=form_error('password')? 'has-error' : null?>">
						...
						<?=form_error('passsword')?>
					</div>

					<div class="form-group <?=form_error('passconf')? 'has-error' : null?>">
						<label>Password Confirmation</label>
						<input type="password" name="passconf" value="<?=$this->input->post('passconf') ?>" class="form-control" placeholder="Enter Password">
						<?=form_error('passconf')?>
					</div>
					...
					<div class="form-group <?=form_error('level')? 'has-error' : null?>">
						...
						<select name="level" class="form-control">
							...
							<option value="2" <?=$level == 2 ? 'selected' : null ?>>Unit</option>
						</select>
						<?=form_error('level')?>
					</div>
